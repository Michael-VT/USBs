...
class App:
    ...
    def build_ui(self) -> None:
        ...
        title = f"Serial IDE v06 ({creation_date}) | Port: {port_info} | Dir: {working_dir}"
        ...
        for i, cmd in enumerate(self.commands):
            self.listbox.insert("end", f"{i+1:02d}: {cmd}")
...
Click HEX → copy to clipboard"""
    ...
    def _handle_timeout(self) -> None:
        ...
        self.log.see("end")
        self.set_status(f"Timeout: no 'Complete' received within {self.seq_timeout}s")
    ...
    def edit_cmd(self, event: tk.Event) -> None:
        ...
        new = simpledialog.askstring("Edit", f"Command {idx+1}:", initialvalue=old)
        if new is not None:
            self.listbox.delete(idx)
            self.listbox.insert(idx, f"{idx+1:02d}: {new}")
